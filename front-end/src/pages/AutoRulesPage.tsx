...
const OPERATOR_OPTIONS: Array<{ value: AutoRuleOperator; label: string }> = [
  { value: 'GT', label: 'Lớn hơn' },
  { value: 'LT', label: 'Bé hơn' },
];

const OPERATOR_LABELS: Record<AutoRuleOperator, string> = {
  GT: 'lớn hơn',
...
const emptyForm: AutoRulePayload = {
  sensorId: '',
  deviceId: '',
  operator: 'GT',
  thresh: 30,
...
const fanIntensityToSpeed = (value: number) => {
  if (value <= 0) return 1;
...
const fanSpeedToIntensity = (speed: number) => {
  if (speed <= 1) return 33;
  if (speed === 2) return 66;
  return 100;
};

const describeDeviceValue = (deviceType: DeviceSummary['deviceType'], value: number) => {
  if (value <= 0) {
    return 'Tắt';
  }
...
export const AutoRulesPage: React.FC = () => {
  ...
  const controlledDeviceCount = new Set(rules.map((rule) => rule.deviceResponse.id)).size;
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div className="mt-2 flex items-end justify-between">
              <span className="text-3xl font-bold text-slate-950 dark:text-white">{rules.length}</span>
            ...
            <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800">
              <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                ...
                <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
                  {activeRuleCount}/{rules.length} đang bật
                </span>
              </div>
            </div>
